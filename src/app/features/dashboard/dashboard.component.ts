...
@Component({
  ...
  template: `
    <app-header title="Dashboard" subtitle="Your financial overview at a glance">
      <button class="btn btn-primary btn-sm" routerLink="/quick-log">
        + Quick Log
      </button>
    </app-header>
    ...
    <div class="dashboard-content">
      ...
      <div class="summary-grid">
        <div class="summary-card income-card">
          ...
          <div class="summary-info">
            ...
            <span class="summary-value text-income">
              {{ txnService.summary().totalIncome | currencyFormat }}
            </span>
        ...
        <div class="summary-card expense-card">
          ...
          <div class="summary-info">
            ...
            <span class="summary-value text-expense">
              {{ txnService.summary().totalExpenses | currencyFormat }}
            </span>
            <span class="summary-sub">{{ currentPeriod }}</span>
          </div>
        </div>

        <div class="summary-card balance-card" [class.positive]="txnService.summary().netBalance >= 0"
             [class.negative]="txnService.summary().netBalance < 0">
          <div class="summary-icon">{{ txnService.summary().netBalance >= 0 ? '💚' : '🔴' }}</div>
          <div class="summary-info">
            <span class="summary-label">Net Balance</span>
            <span class="summary-value" [class.text-income]="txnService.summary().netBalance >= 0"
      ...
      <div class="charts-row">
        ...
        <div class="card chart-card chart-card-wide">
          <div class="card-header">
            <span class="card-title">Monthly Trend ({{ currentYear }})</span>
          </div>
      ...
      <div class="bottom-row">
        ...
        <div class="card recent-card">
          ...
          @if (txnService.loading()) {
            ...
            <div class="empty-state">
              ...
              <a routerLink="/quick-log" class="btn btn-primary btn-sm">Quick Log Transaction</a>
            </div>
          } @else {
            <div class="txn-list">
              @for (txn of txnService.recentTransactions(); track txn.id) {
                <div class="txn-item">
                  ...
                  <div class="txn-details">
                    ...
                    <span class="txn-meta">{{ txn.category }} · {{ txn.date }}</span>
                  </div>
                  <span class="txn-amount" [class.text-income]="txn.type === 'income'"
                ...
                <div class="budget-item">
                  ...
                  <div class="progress-bar">
                    <div class="progress-fill"
                         ...
                         [style.background]="alert.status === 'exceeded' ? 'var(--accent-red)' :
                                             alert.status === 'warning' ? 'var(--accent-yellow)' :
                                             'var(--accent-green)'">
                    </div>
                  </div>
        ...
        <div class="card category-card">
          <div class="card-header">
            <span class="card-title">Top Spending</span>
          </div>
          <div class="chart-container-sm">
...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  getCategoryIcon(categoryId: string): string {
    return this.categoryService.getCategoryIcon(categoryId);
  }
  ...
  private initDoughnutChart() {
    ...
    this.doughnutChart = new Chart(this.doughnutRef.nativeElement, {
      ...
      options: {
        ...
        plugins: {
          legend: { position: 'bottom', labels: { color: '#9fa8da', font: { size: 11 } } },
  ...
  private initPieChart() {
    ...
    this.pieChart = new Chart(this.pieRef.nativeElement, {
      ...
      data: {
        labels,
        datasets: [{ data, backgroundColor: colors, borderWidth: 1, borderColor: 'var(--bg-card)' }]
      },
      options: {
        responsive: true,
        maintainAspectRatio: false,
        plugins: {
          legend: { position: 'bottom', labels: { color: '#9fa8da', font: { size: 10 }, boxWidth: 12 } },
          tooltip: { callbacks: { label: ctx => ` $${ctx.parsed.toLocaleString()}` } }
        }
      }
    });
  ...
  private getCategoryData() {
    const expenses = this.txnService.transactions().filter(t => t.type === 'expense');
    ...
    const labels = sorted.map(([cat]) => this.categoryService.getCategoryById(cat)?.name ?? cat);
    const data = sorted.map(([, v]) => v);
    const colors = sorted.map(([cat]) => this.categoryService.getCategoryColor(cat));
